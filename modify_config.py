...
# ====================================================================
# 专属引流 QQ 群（保留在旧版本扫描轰炸兼容中）
MY_QQ_GROUP = "532637640"

# 专属引流 Telegram 频道（用于推送通知与过期大轰炸提示中）
...
# FongMi 专属高清带群号水印壁纸（全量版/纯净版可分开定制）
...
# ====================================================================
old_configs = glob.glob(f'datas/{BASE_OUTPUT_FULL}*.json') + glob.glob(f'datas/{BASE_OUTPUT_CLEAN}*.json') + glob.glob('datas/蝴蝶影视*.json') + glob.glob('datas/老杨TV*.json')
    ...
    if old_base != full_output_filename and old_base != clean_output_filename:
        try:
            trap_json = {
                ...
                "warningText": TRAP_WARNING_TEXT,
                "sites": [
                    {"key": "蝴蝶纯文字提示", "name": TRAP_SITE_NAME_1, "type": 3, "api": "csp_JuDou", "searchable": 0, "quickSearch": 0, "filterable": 0},
                    {"key": "蝴蝶纯文字提示2", "name": TRAP_SITE_NAME_2, "type": 3, "api": "csp_JuDou", "searchable": 0, "quickSearch": 0, "filterable": 0}
                ],
                "lives": [
...
try:
    ...
    try:
        ...
        if "rules" in ordered_obj and isinstance(ordered_obj["rules"], list):
            ...
            js_injection_rule = {"name": "蝴蝶影视·云端高级去广告JS注入", "hosts": ad_hosts, "script": CUSTOM_AD_BLOCK_JS}
    ...
    if is_password_changed or is_new_token_generated:
        try:
            ...
            pwd_msg += f"👑 矩阵连接已在后台全自动换锁，请及时前往电视端更新。若电视端遇到断流请尝试重启软件或前往TG频道（{MY_PROMO_CHANNEL}）获取支持！"
